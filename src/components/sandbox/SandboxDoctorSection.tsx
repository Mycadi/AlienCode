...
export function SandboxDoctorSection() {
  ...
  if ($[0] === Symbol.for("react.memo_cache_sentinel")) {
    ...
    bb0: {
      ...
      t0 = <Box flexDirection="column"><Text bold={true}>Sandbox</Text><Text>{`${TREE_END} `}Status: <Text color={statusColor}>{statusText}</Text></Text>{depCheck.errors.map(_temp)}{depCheck.warnings.map(_temp2)}{hasErrors && <Text dimColor={true}>{`${TREE_END} `}Run /sandbox for install instructions</Text>}</Box>;
    }
    $[0] = t0;
    $[1] = t1;
  } else {
...
function _temp2(w, i_0) {
  return <Text key={i_0} color="warning">{`${TREE_END} `}{w}</Text>;
}
function _temp(e, i) {
  return <Text key={i} color="error">{`${TREE_END} `}{e}</Text>;
}
