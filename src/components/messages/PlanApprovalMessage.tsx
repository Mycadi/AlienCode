...
export function PlanApprovalResponseDisplay(t0) {
  ...
  if (response.approved) {
    let t1;
    if ($[0] !== senderName) {
      t1 = <Box><Text color="success" bold={true}>{CHECK} Plan Approved by {senderName}</Text></Box>;
      $[0] = senderName;
      $[1] = t1;
    } else {
      t1 = $[1];
  ...
  let t1;
  if ($[5] !== senderName) {
    t1 = <Box><Text color="error" bold={true}>{CROSS} Plan Rejected by {senderName}</Text></Box>;
    $[5] = senderName;
    $[6] = t1;
  } else {
    t1 = $[6];
