...
export function ModelPicker(t0) {
  ...
  const [codexLimitsText, setCodexLimitsText] = useState(() => getCodexOAuthTokens() ? getCodexLimitsDisplayText() : undefined);
  ...
  const shouldShowCodexLimits = focusedModel ? isCodexModel(focusedModel) : false;
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    refreshCodexLimitsFromAnalytics(tokens.accessToken, tokens.accountId).then(() => {
      if (!cancelled) {
        setCodexLimitsText(getCodexLimitsDisplayText());
      }
    });
  ...
  const t27 = shouldShowCodexLimits && codexLimitsText && <Box marginBottom={1}><Text dimColor={true}>{codexLimitsText}</Text></Box>;
  let t28;
  if ($[72] !== t19 || $[73] !== t23 || $[74] !== t24 || $[75] !== t25 || $[76] !== t27) {
    t28 = <Box flexDirection="column">{t19}{t23}{t24}{t25}{t27}</Box>;
    $[72] = t19;
    $[73] = t23;
    $[74] = t24;
    $[75] = t25;
  ...
  let t30;
  if ($[81] !== t28 || $[82] !== t29) {
    t30 = <Box flexDirection="column">{t28}{t29}</Box>;
    $[81] = t28;
    $[82] = t29;
    $[83] = t30;
  } else {
  ...
  const hasCompactInset = !!process.env.CLAUDE_CODE_NUI || !!process.env.CLAUDE_CODE_MINIMAL;
  let t31;
  if ($[86] !== content || $[87] !== hasCompactInset) {
    t31 = hasCompactInset ? <Box flexDirection="column" paddingX={2}>{content}</Box> : <Pane color="permission">{content}</Pane>;
    $[86] = content;
    $[87] = hasCompactInset;
    $[88] = t31;
  } else {
