...
import { formatLogoModelSource, formatModelAndBilling, getLogoDisplayData, truncatePath } from '../../utils/logoV2Utils.js';
...
export function CondensedLogo() {
  ...
  const textWidth = Math.max(columns - 22, 20);
  const truncatedVersion = truncate(version, Math.max(textWidth - 13, 6));
  const {
    modelName,
    sourceName
  } = formatLogoModelSource(model, modelDisplayName, effortValue, billingType);
  const {
    shouldSplit,
    truncatedModel,
    truncatedBilling
  } = sourceName ? formatModelAndBilling(modelName, sourceName, textWidth) : {
    shouldSplit: false,
    truncatedModel: truncate(modelName, textWidth),
    truncatedBilling: ''
  };
  ...
  if ($[8] === Symbol.for("react.memo_cache_sentinel")) {
    t5 = <Text bold={true}>Alien Code</Text>;
    $[8] = t5;
  } else {
    t5 = $[8];
  }
  ...
  if ($[11] !== shouldSplit || $[12] !== truncatedBilling || $[13] !== truncatedModel) {
    t7 = sourceName ? shouldSplit ? <><Text dimColor={true}>{truncatedModel}</Text><Text dimColor={true}>{truncatedBilling}</Text></> : <Text dimColor={true}>{truncatedModel} · {truncatedBilling}</Text> : <Text dimColor={true}>{truncatedModel}</Text>;
    $[11] = shouldSplit;
    $[12] = truncatedBilling;
    $[13] = truncatedModel;
    $[14] = t7;
  ...
  if ($[23] !== t10 || $[24] !== t11 || $[25] !== t6 || $[26] !== t7 || $[27] !== t9) {
    t12 = isMinimal
      ? <OffscreenFreeze><Box flexDirection="column" marginTop={1} marginLeft={1} marginRight={1}>{t6}{t7}{t9}{t10}{t11}</Box></OffscreenFreeze>
      : <OffscreenFreeze><Box flexDirection="row" gap={2} alignItems="center" marginTop={1} marginLeft={1} marginRight={1}>{t4}<Box flexDirection="column">{t6}{t7}{t9}{t10}{t11}</Box></Box></OffscreenFreeze>;
    $[23] = t10;
    $[24] = t11;
    $[25] = t6;
    $[26] = t7;
