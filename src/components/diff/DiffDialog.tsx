...
import { TRI_L, TRI_R } from '../../utils/nuiChars.js';
...
export function DiffDialog(t0) {
  ...
  if ($[40] !== sourceIndex || $[41] !== sources) {
    t18 = sources.length > 1 ? <Box>{sourceIndex > 0 && <Text dimColor={true}>{`${TRI_L} `}</Text>}{sources.map((source, i) => {
      ...
      })}{sourceIndex < sources.length - 1 && <Text dimColor={true}>{` ${TRI_R}`}</Text>}</Box> : null;
    $[40] = sourceIndex;
    $[41] = sources;
    $[42] = t18;
  } else {
