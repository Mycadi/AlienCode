...
function PromptInput({
  ...
  const hasCompactInset = isNui || isMinimal;
  const promptColumns = hasCompactInset ? Math.max(columns - 2, 0) : columns;
  const promptWidth = hasCompactInset ? promptColumns : "100%";
  if (isExternalEditorActive) {
    return <Box marginLeft={hasCompactInset ? 1 : 0} marginRight={hasCompactInset ? 1 : 0}><Box flexDirection="row" alignItems="center" justifyContent="center" borderColor={getBorderColor()} borderStyle="round" borderLeft={false} borderRight={false} borderBottom width={promptWidth}>
  ...
  return <Box flexDirection="column" marginTop={briefOwnsGap ? 0 : 1} marginLeft={hasCompactInset ? 1 : 0} marginRight={hasCompactInset ? 1 : 0}>
      ...
      {swarmBanner ? <>
          <Text color={swarmBanner.bgColor}>
            {swarmBanner.text ? <>
                {(isNui ? '-' : '─').repeat(Math.max(0, promptColumns - stringWidth(swarmBanner.text) - 4))}
                <Text backgroundColor={swarmBanner.bgColor} color="inverseText">
                  {' '}
                  {swarmBanner.text}{' '}
                </Text>
                {isNui ? '--' : '──'}
              </> : (isNui ? '-' : '─').repeat(promptColumns)}
          </Text>
          <Box flexDirection="row" width={promptWidth}>
            <PromptInputModeIndicator mode={mode} isLoading={isLoading} viewingAgentName={viewingAgentName} viewingAgentColor={viewingAgentColor} />
            <Box flexGrow={1} flexShrink={1} onClick={handleInputClick}>
              {textInputElement}
            </Box>
          </Box>
          <Text color={swarmBanner.bgColor}>{(isNui ? '-' : '─').repeat(promptColumns)}</Text>
        </> : isMinimal ? <Box flexDirection="row" width={promptWidth}>
          <PromptInputModeIndicator mode={mode} isLoading={isLoading} viewingAgentName={viewingAgentName} viewingAgentColor={viewingAgentColor} />
          <Box flexGrow={1} flexShrink={1} onClick={handleInputClick}>
            {textInputElement}
          </Box>
        </Box> : <Box flexDirection="row" alignItems="flex-start" justifyContent="flex-start" borderColor={getBorderColor()} borderStyle="round" borderLeft={false} borderRight={false} borderBottom width={promptWidth} borderText={buildBorderText(showFastIcon ?? false, showFastIconHint, fastModeCooldown)}>
          <PromptInputModeIndicator mode={mode} isLoading={isLoading} viewingAgentName={viewingAgentName} viewingAgentColor={viewingAgentColor} />
          <Box flexGrow={1} flexShrink={1} onClick={handleInputClick}>
            {textInputElement}
          </Box>
