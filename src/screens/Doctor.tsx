...
import { TREE_END } from '../utils/nuiChars.js';
...
function DistTagsDisplay(t0) {
  ...
  if (!distTags.latest) {
    ...
    if ($[0] === Symbol.for("react.memo_cache_sentinel")) {
      t1 = <Text dimColor={true}>{`${TREE_END} `}Failed to fetch versions</Text>;
      $[0] = t1;
    } else {
      t1 = $[0];
    }
  ...
  if ($[1] !== distTags.stable) {
    t1 = distTags.stable && <Text>{`${TREE_END} `}Stable version: {distTags.stable}</Text>;
    $[1] = distTags.stable;
    $[2] = t1;
  } else {
    t1 = $[2];
  ...
  if ($[3] !== distTags.latest) {
    t2 = <Text>{`${TREE_END} `}Latest version: {distTags.latest}</Text>;
    $[3] = distTags.latest;
    $[4] = t2;
  } else {
    t2 = $[4];
...
export function Doctor(t0) {
  ...
  if ($[18] !== diagnostic.installationType || $[19] !== diagnostic.version) {
    t11 = <Text>{`${TREE_END} `}Currently running: {diagnostic.installationType} ({diagnostic.version})</Text>;
  ...
  if ($[21] !== diagnostic.packageManager) {
    t12 = diagnostic.packageManager && <Text>{`${TREE_END} `}Package manager: {diagnostic.packageManager}</Text>;
  ...
  if ($[23] !== diagnostic.installationPath) {
    t13 = <Text>{`${TREE_END} `}Path: {diagnostic.installationPath}</Text>;
  ...
  if ($[25] !== diagnostic.invokedBinary) {
    t14 = <Text>{`${TREE_END} `}Invoked: {diagnostic.invokedBinary}</Text>;
  ...
  if ($[27] !== diagnostic.configInstallMethod) {
    t15 = <Text>{`${TREE_END} `}Config install method: {diagnostic.configInstallMethod}</Text>;
  ...
  let t18;
  if ($[29] !== t16 || $[30] !== t17) {
    t18 = <Text>{`${TREE_END} `}Search: {t16} ({t17})</Text>;
    $[29] = t16;
    $[30] = t17;
    $[31] = t18;
  } else {
  ...
  let t26;
  if ($[52] !== t25) {
    t26 = <Text>{`${TREE_END} `}Auto-updates:{" "}{t25}</Text>;
    $[52] = t25;
    $[53] = t26;
  } else {
    t26 = $[53];
  ...
  if ($[54] !== diagnostic.hasUpdatePermissions) {
    t27 = diagnostic.hasUpdatePermissions !== null && <Text>{`${TREE_END} `}Update permissions:{" "}{diagnostic.hasUpdatePermissions ? "Yes" : "No (requires sudo)"}</Text>;
  ...
  if ($[56] === Symbol.for("react.memo_cache_sentinel")) {
    t28 = <Text>{`${TREE_END} `}Auto-update channel: {autoUpdatesChannel}</Text>;
    $[56] = t28;
  } else {
    t28 = $[56];
  }
  ...
  if ($[65] !== versionLockInfo) {
    t35 = versionLockInfo?.enabled && <Box flexDirection="column"><Text bold={true}>Version Locks</Text>{versionLockInfo.staleLocksCleaned > 0 && <Text dimColor={true}>{`${TREE_END} `}Cleaned {versionLockInfo.staleLocksCleaned} stale lock(s)</Text>}{versionLockInfo.locks.length === 0 ? <Text dimColor={true}>{`${TREE_END} `}No active version locks</Text> : versionLockInfo.locks.map(_temp12)}</Box>;
    $[65] = versionLockInfo;
    $[66] = t35;
  } else {
    t35 = $[66];
  ...
  let t36;
  if ($[67] !== agentInfo) {
    t36 = agentInfo?.failedFiles && agentInfo.failedFiles.length > 0 && <Box flexDirection="column"><Text bold={true} color="error">Agent Parse Errors</Text><Text color="error">{`${TREE_END} `}Failed to parse {agentInfo.failedFiles.length} agent file(s):</Text>{agentInfo.failedFiles.map(_temp13)}</Box>;
    $[67] = agentInfo;
    $[68] = t36;
  } else {
    t36 = $[68];
  ...
  let t37;
  if ($[69] !== pluginsErrors) {
    t37 = pluginsErrors.length > 0 && <Box flexDirection="column"><Text bold={true} color="error">Plugin Errors</Text><Text color="error">{`${TREE_END} `}{pluginsErrors.length} plugin error(s) detected:</Text>{pluginsErrors.map(_temp14)}</Box>;
    $[69] = pluginsErrors;
    $[70] = t37;
  } else {
    t37 = $[70];
  ...
  if ($[71] !== contextWarnings) {
    t38 = contextWarnings?.unreachableRulesWarning && <Box flexDirection="column"><Text bold={true} color="warning">Unreachable Permission Rules</Text><Text>{`${TREE_END} `}<Text color="warning">{figures.warning}{" "}{contextWarnings.unreachableRulesWarning.message}</Text></Text>{contextWarnings.unreachableRulesWarning.details.map(_temp15)}</Box>;
    $[71] = contextWarnings;
    $[72] = t38;
  } else {
    t38 = $[72];
  ...
  if ($[73] !== contextWarnings) {
    t39 = contextWarnings && (contextWarnings.claudeMdWarning || contextWarnings.agentWarning || contextWarnings.mcpWarning) && <Box flexDirection="column"><Text bold={true}>Context Usage Warnings</Text>{contextWarnings.claudeMdWarning && <><Text>{`${TREE_END} `}<Text color="warning">{figures.warning} {contextWarnings.claudeMdWarning.message}</Text></Text><Text>{"  "}{`${TREE_END} `}Files:</Text>{contextWarnings.claudeMdWarning.details.map(_temp16)}</>}{contextWarnings.agentWarning && <><Text>{`${TREE_END} `}<Text color="warning">{figures.warning} {contextWarnings.agentWarning.message}</Text></Text><Text>{"  "}{`${TREE_END} `}Top contributors:</Text>{contextWarnings.agentWarning.details.map(_temp17)}</>}{contextWarnings.mcpWarning && <><Text>{`${TREE_END} `}<Text color="warning">{figures.warning} {contextWarnings.mcpWarning.message}</Text></Text><Text>{"  "}{`${TREE_END} `}MCP servers:</Text>{contextWarnings.mcpWarning.details.map(_temp18)}</>}</Box>;
    $[73] = contextWarnings;
    $[74] = t39;
  } else {
    t39 = $[74];
...
function _temp18(detail_2, i_8) {
  return <Text key={i_8} dimColor={true}>{"    "}{`${TREE_END} `}{detail_2}</Text>;
}
function _temp17(detail_1, i_7) {
  return <Text key={i_7} dimColor={true}>{"    "}{`${TREE_END} `}{detail_1}</Text>;
}
function _temp16(detail_0, i_6) {
  return <Text key={i_6} dimColor={true}>{"    "}{`${TREE_END} `}{detail_0}</Text>;
}
function _temp15(detail, i_5) {
  return <Text key={i_5} dimColor={true}>{"  "}{`${TREE_END} `}{detail}</Text>;
}
function _temp14(error_0, i_4) {
  return <Text key={i_4} dimColor={true}>{"  "}{`${TREE_END} `}{error_0.source || "unknown"}{"plugin" in error_0 && error_0.plugin ? ` [${error_0.plugin}]` : ""}:{" "}{getPluginErrorMessage(error_0)}</Text>;
}
function _temp13(file, i_3) {
  return <Text key={i_3} dimColor={true}>{"  "}{`${TREE_END} `}{file.path}: {file.error}</Text>;
}
function _temp12(lock, i_2) {
  return <Text key={i_2}>{`${TREE_END} `}{lock.version}: PID {lock.pid}{" "}{lock.isProcessRunning ? <Text>(running)</Text> : <Text color="warning">(stale)</Text>}</Text>;
}
function _temp11(validation, i_1) {
  return <Text key={i_1}>{`${TREE_END} `}{validation.name}:{" "}<Text color={validation.status === "capped" ? "warning" : "error"}>{validation.message}</Text></Text>;
}
...
function _temp1(install, i) {
  return <Text key={i}>{`${TREE_END} `}{install.type} at {install.path}</Text>;
}
function _temp0(a) {
  return {
