...
export function It2SetupPrompt(t0) {
  ...
  if ($[13] !== error || $[14] !== handleInstall || $[15] !== handleUseTmux || $[16] !== onDone || $[17] !== packageManager || $[18] !== step || $[19] !== tmuxAvailable) {
    ...
    function renderApiInstructions() {
      ...
      return <Box flexDirection="column" gap={1}><Text color="success">{CHECK} it2 installed successfully</Text><Box flexDirection="column" marginTop={1}>{instructions.map(_temp)}</Box><Box marginTop={1}><Text dimColor={true}>Press Enter when ready to verify…</Text></Box></Box>;
    }
    function renderVerifying() {
      return <Box><Spinner /><Text> Verifying it2 can communicate with iTerm2…</Text></Box>;
    }
    function renderSuccess() {
      return <Box flexDirection="column"><Text color="success">{CHECK} iTerm2 split pane support is ready</Text><Text dimColor={true}>Teammates will now appear as split panes.</Text></Box>;
    }
