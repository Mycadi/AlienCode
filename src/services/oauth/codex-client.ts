...
import { OK_BOX } from '../../utils/nuiChars.js';
  ...
  return new Promise<LocalServer>((resolve) => {
    const s = createServer((req, res) => {
      try {
        ...
        res.end(
          '<html><body><h2>${OK_BOX} OpenAI authentication completed.</h2><p>You can close this window and return to your terminal.</p></body></html>',
        )
        settleWait?.({ code })
        settleWait = null
      } catch {
