...
function SpeechBubble(t0) {
  ...
  if (tail === "right") {
    let t8;
    if ($[21] !== borderColor) {
      t8 = <Text color={borderColor}>{isNui ? "-" : "─"}</Text>;
      $[21] = borderColor;
      $[22] = t8;
    } else {
      t8 = $[22];
  ...
  let t8;
  if ($[26] !== borderColor) {
    t8 = <Box flexDirection="column" alignItems="flex-end" paddingRight={6}><Text color={borderColor}>{isNui ? "\ " : "╲ "}</Text><Text color={borderColor}>{isNui ? "\\" : "╲"}</Text></Box>;
    $[26] = borderColor;
    $[27] = t8;
  } else {
    t8 = $[27];
