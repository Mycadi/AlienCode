    ...
    for (let y = 0; y < screen.height; y++) {
      ...
      for (let x = 0; x < screen.width; x++) {
        ...
        if (cell && cell.width !== CellWidth.SpacerTail) {
          ...
          if (!(process.env.CLAUDE_CODE_MINIMAL || process.env.CLAUDE_CODE_NUI) && cell.hyperlink !== currentHyperlink) {
      ...
      if (!(process.env.CLAUDE_CODE_MINIMAL || process.env.CLAUDE_CODE_NUI) && currentHyperlink !== undefined) {
        line += LINK_END
...
function transitionHyperlink(
  ...
  if (process.env.CLAUDE_CODE_MINIMAL || process.env.CLAUDE_CODE_NUI) return current
  if (current !== target) {
