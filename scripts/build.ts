...
const outfile = nodeTarget
  ? dev
    ? './dist/acode-node-dev.mjs'
    : './dist/acode-node.mjs'
  : compile
    ? dev
      ? './dist/acode-dev'
...
const defines = {
  ...
  ...(dev
    ...
    : {}),
  ...(nodeTarget
    ? { 'process.env.CLAUDE_CODE_NUI': JSON.stringify('1'),
        'process.env.CLAUDE_CODE_MINIMAL': JSON.stringify('1') }
    : {}),
...
if (!nodeTarget && existsSync(outfile)) {
